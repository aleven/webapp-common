...
public class DbUtilsConnector extends JdbcConnector {
	...
	// Logger.getLogger(DbUtilsConnector.class.getName());

	// private static final String QUERY_DISEGNO = "SELECT * " + "FROM " + "(" +
	// "SELECT ntext2 AS nome, float1 AS revNum, SUBSTRING(CAST(ntext2 AS nvarchar(max)), LEN(CAST(ntext2 AS nvarchar(max)))-4, 1) AS revOnName FROM UserData "
	// + "WHERE " + "(" + "ntext2 LIKE '%__.__.____.___._.dft' " + "OR " +
	// "ntext2 LIKE '%__.__.____.___._-_.dft' " + ")" + "AND " +
	// "nvarchar10 IN ('Rilasciato', 'En diffusion') " + ") AS dft " + "WHERE "
	// + "nome LIKE '%{0}%' " + "ORDER BY revOnName DESC";
	// private static final String QUERY_DISEGNO_TOP1 = "SELECT TOP 1 * " +
	// "FROM " + "(" +
	// "SELECT ntext2 AS nome, float1 AS revNum, SUBSTRING(CAST(ntext2 AS nvarchar(max)), LEN(CAST(ntext2 AS nvarchar(max)))-4, 1) AS revOnName FROM UserData "
	// + "WHERE " + "(" + "ntext2 LIKE '%__.__.____.___._.dft' " + "OR " +
	// "ntext2 LIKE '%__.__.____.___._-_.dft' " + ")" + "AND " +
	...
	// + "nome LIKE '%{0}%' " + "ORDER BY revOnName DESC";
	//
	// private static final String QUERY_DISEGNO_ALL = "SELECT * " + "FROM " +
	// "(" +
	// "SELECT ntext2 AS nome, float1 AS revNum, SUBSTRING(CAST(ntext2 AS nvarchar(max)), LEN(CAST(ntext2 AS nvarchar(max)))-4, 1) AS revOnName FROM UserData "
	// + "WHERE " + "(" + "ntext2 LIKE '%__.__.____.___._.dft' " + "OR " +
	// "ntext2 LIKE '%__.__.____.___._-_.dft' " + ")" + "AND " +
	// "nvarchar10 IN ('Rilasciato', 'En diffusion') " + ") AS dft " +
	// "ORDER BY revOnName DESC";
	//
	// private static final String QUERY_DISEGNO_TEST = "SELECT * " + "FROM " +
	// "(" +
	// "SELECT ntext2 AS nome, float1 AS revNum, SUBSTRING(CAST(ntext2 AS nvarchar(max)), LEN(CAST(ntext2 AS nvarchar(max)))-4, 1) AS revOnName FROM UserData "
	// + "WHERE " + "(" + "ntext2 LIKE '%__.__.____.___._.dft' " + "OR " +
	// "ntext2 LIKE '%__.__.____.___._-_.dft' " + ")" + "AND " +
	// "nvarchar10 IN ('Rilasciato', 'En diffusion') " + ") AS dft " + "WHERE "
	// + "nome LIKE '%28.63.0024.000.0%' " + "ORDER BY revOnName DESC";
	...
	// public DbUtilsConnector(String DB_CONN_STRING, String DRIVER_CLASS_NAME,
	...
	// * @throws SQLException
	// */
	// public List<T> cercaDisegni(String numeroDisegno, Class<T> clazz) throws
	// SQLException {
	...
	// */
	// result = run.query(getConnection(), formatLikeCSharp(QUERY_DISEGNO,
	// numeroDisegno), getFiller(clazz));
	//
	...
	public int executeUpdate(boolean keepConnOpen, String aQuery) throws Exception {
		...
		try {
			...
			// int inserts = run.update(getConnection(),
			// "INSERT INTO Person (name,height) VALUES (?,?)", "John Doe",
			// 1.82);

			// res = run.update(getConnection(), aQuery, params);
			...
			// int updates =
			// run.update("UPDATE Person SET height=? WHERE name=?", 2.05,
			// "John Doe");
	...
	// public ResultSet executeSelect(boolean keepConnOpen, String aQuery)
	// throws SQLException {
	// ResultSet res = null;
	//
	// // QueryRunner run = new QueryRunner();
